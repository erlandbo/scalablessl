...
class SCL(L.LightningModule):
    def __init__(self,
        ...
        if modelname == "resnet":
            self.backbone = ResNet(in_channels=3)
        else:
        ...
        self.register_buffer("ro", torch.zeros(1,) + ro)  # [0,1] forgetting rate s_inv
    ...
    def _sim_metric(self, z1, z2):
        if self.hparams.simmetric == "stud-tkernel":
            ...
            sig = 2.0  # TODO how find suitable variance?
    ...
    def load_knn_finetune_dataset(self, traindataset, testdataset):
        self.finetune_trainloader = DataLoader(dataset=traindataset, batch_size=512, shuffle=True, num_workers=self.hparams.num_workers)
        self.finetune_testloader = DataLoader(dataset=testdataset, batch_size=512, shuffle=False, num_workers=self.hparams.num_workers)
...
if __name__ == "__main__":
    ...
    traindataset, valdataset, testdataset, mean, std = load_imagedataset(datasetname=DATASET, val_split=VAL_SPLIT)
    ...
    finetune_traindataset = SCLFinetuneDataset(
        traindataset,
        transform=SCLEvalTransform(imgsize=IMG_SIZE, mean=mean, std=std, num_views=1),
    )
