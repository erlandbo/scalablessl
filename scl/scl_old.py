...
import torch
from scl.ConvLayers import ResBlock
...
import numpy as np
from scl.Augmentations import SwavTrainTransform, SwavEvalTransform
...
class SCL(L.LightningModule):
    ...
    def on_train_epoch_start(self) -> None:
        ...
        self.xi = torch.zeros(1,)
        self.omega = torch.zeros(1,)
    ...
    #     x_i, xhat_i, xhat_j, y = batch
    #     z = self.forward(torch.cat([x_i, xhat_i, xhat_j], dim=0))
    #     B = x_i.shape[0]  # batchsize
    #     # TODO parametric or non-parametric similarity?
    #     q = (z @ z.T) / (z.norm(p=2, dim=1, keepdim=True) @ z.norm(p=2, dim=1, keepdim=True).T)
    #     q = q * 0.5 + 0.5  # cosine similarity [-1,1] -> [0,1]
    #     # positive forces
    #     mask = torch.eye(q.shape[0], dtype=torch.bool, device=z.device)
    #     pos_mask = mask.roll(shifts=B, dims=1).bool()  # shift eye to find positive B-places away after stacking
    #     qii = q[pos_mask]
    #     positive_forces = - torch.log(qii)
    #     self.xi = self.xi + self.alpha * qii
    #     self.omega = self.omega + self.alpha
    #     # negative forces
    #     # TODO sample more cleanly
    #     q_flatten = torch.flatten(q)
    #     ij = torch.randperm(q_flatten.shape[0])
    #     q_shuffled = q_flatten[ij]
    #     qij = q_shuffled[0:B] if B > 2 else q_shuffled[0:1]  # TODO clean hack for batchsize 1
    #     negative_forces = qij / self.s_inv
    #     self.xi = xi + (1 - self.alpha) * qij
    ...
    def _shared_step(self, batch, batch_idx, mode="train"):
        ...
        negative_forces = qij / self.s_inv
        self.xi = self.xi + (1 - self.alpha) * qij
        self.omega = self.omega + (1 - self.alpha)
...
class SCLDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        x_j = self.transform(self.basedataset[j])
        return x_i, xhat_i, x_j, y
...
if __name__ == "__main__":
    ...
    # HYPERPARAMS
    # TODO avoid hack for batchsize=1
    BATCH_SIZE = 2  # use 2 for batchsize=1 since else no negatives
    NUM_WORKERS = 20
    MAX_EPOCHS = 100
    OPTIMIZER_NAME = "sgd"  #
    LR = 3e-4  # lr?
